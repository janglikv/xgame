...
import { bodyProfileIdForKind } from './creatureKinds';
...
export type { CreatureKind } from './creatureKinds';
...
export type WorldCreatureOptions = {
  scale?: number;
  maxHp?: number;
  /** 物种 id（强类型）；缺省 spider */
  kind?: CreatureKind;
...
/**
 * 世界生物基类：世界坐标、贴图、血量、击飞、巡视/追击/扑咬 AI、生态注入。
 * 蜘蛛 / 农场动物 / 木桩等均继承本类；物种用 `kind` 区分，不用 label 字符串协议。
...
export class WorldCreature extends Container implements WorldActor {
  ...
  constructor(worldX: number, worldY: number, options: WorldCreatureOptions = {}) {
    super();
    this.appearance = {
    ...
    this.kind = options.kind ?? 'spider';
    this.bodyProfileId = bodyProfileIdForKind(this.kind);
